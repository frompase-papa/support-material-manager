...
|---|---|---|
| `📡 記録中` | 緑 | 正常。未送信はゼロ |
| `▶ 開始を送信` / `✓ 送信: …` | 青 | 送信できた（2.8秒だけ表示） |
| **`⚠ 未送信 N件`** | **赤** | **送れていないデータが残っている。ゼロになるまで出続ける** |

赤が出たままの場合は、アプリ側（Vercel）の `STUDY_API_KEY` と `config.js` の
`API_KEY` が食い違っていないか、ネットワークが繋がっているかを確認してください。

### 送信に失敗したときの動き（v1.4.0〜）
送れなかったデータは端末内（`chrome.storage.local`）に保存し、**約20秒ごとに自動で再送**します。
鍵の設定ミスや通信断があっても、直せば**溜まっていたぶんがまとめて送られます**。

- 再送するのは「あとで直る可能性がある失敗」（通信断・401・403・5xx）です。
- `400` など**内容そのものが不正な場合は再送しません**（何度送っても通らないため）。
- 上限は300件、7日を過ぎたものは諦めて捨てます。

> ⚠ v1.3.0 以前は再送しません。2026-09-15 に、鍵の不一致で丸一日ぶんの記録が失われました。
